$(function(){
	...
	//======== SHOW PASSWORD JS ========//
	 ...
	 //  });
	  ...
	  //======= AJAX ORDER BUKU ======//
	  $(document).on('change','select[name="order-buku"]',function(){
	  	console.log('sip');
			var cari       = $('input[name="cari"]').val();
			var order_buku = $(this).val();
			if (cari != '') {
				// window.href.location = 'buku/order/'+order_buku+'/cari?cari='+cari;
				window.location.href = window.location.href+'/order/'+order_buku+'/cari?cari='+cari;
			}
			else {
				window.location.href = window.location.href+'/order/'+order_buku;
			}
			// var getUrl     = '';
			// if (cari != '') {
